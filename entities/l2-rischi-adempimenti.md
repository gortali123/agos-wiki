...
- **`ristrutturazioni_o`** — **bug nel nome file**: `ristrutturazioni_o_sql` / `ristrutturazioni_o_yml` (mancano i punti prima dell'estensione). Come nominati, dbt non li riconoscerebbe come file modello validi. Il contenuto yml interno è comunque corretto (`query_tag` allineato all'area). Vedi [[inconsistenze]].
