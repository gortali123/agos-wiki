...
- **Naming**: la vecchia struttura `.old` usava ancora `PROGRESSIVO_PK`/`PROGRESSIVO_CONTROPARTE` (pre-rename 2026-07-20). Nel fix sono stati portati alla convenzione corrente `PR_PK`/`PR_CONTROPARTE` (quest'ultimo non ancora rinominato nemmeno in `raw/dwh-code/`, dove resta `PROGRESSIVO_CONTROPARTE` per esteso — vedi [[naming-convention-agos-x]]/[[inconsistenze]] voce 4; il develop yml è stato aggiornato di conseguenza).
