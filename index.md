...
- [primary_key_from_sql (nuovo test)](develop/tests/generic/primary_key_from_sql.sql) — analogo a `try_cast_from_sql` ma per PK: `null_pks`/`duplicate_pks` invariate (agiscono sui valori grezzi), `cast_failed_pks` riusa il parsing del `raw_code` L1 per testare l'espressione di cast reale della/e colonna/e PK invece di un `TRY_CAST(col AS tipo_dichiarato)` ricostruito a mano — necessario quando la colonna PK ha logica di cast particolare (es. `TRY_TO_DATE`/`TRY_TO_TIMESTAMP` con formato custom). Da usare al posto di `primary_key` quando una o più `pk_columns` hanno questa logica; proposto
- [granulaires_v0/v1/vf (where_clause)](develop/models/L0/BSN/granulaires_v0_source.yml) — L1 filtra `WHERE enreg = '01'` (colonna sorgente diretta), il test `try_cast_from_sql` non lo replicava: aggiunto `where_clause: "enreg = '01'"` come argomento in tutti e 3 i source yml (v0/v1/vf), proposto
- [flux_cre_provision (where_clause)](develop/models/L0/BSN/flux_cre_provision_source.yml) — L1 filtra `WHERE energ = '01'`, aggiunto `where_clause: "energ = '01'"` a `try_cast_from_sql`; nota: questa tabella ha anche un mismatch di variante già segnalato nell'audit (SQL semplice, dovrebbe usare `try_cast` non `try_cast_from_sql` — non corretto qui, resta un fix separato), proposto
- [mailc_esiti_tgb (where_clause)](develop/models/L0/POSTE/mailc_esiti_tgb_source.yml) — L1 filtra `WHERE TRY_TO_DATE(DT_ACCETTAZIONE,'DDMMYYYY') = get_dt_osservazione('weekly')`, aggiunto lo stesso `where_clause` a `try_cast_from_sql`; da riallineare se/quando la proposta di reclassificazione a cluster D in `develop/models/L1/POSTE/mailc_esiti_tgb.yml` (che cambia la condizione in `get_dt_accettazione()`) viene applicata upstream, proposto
