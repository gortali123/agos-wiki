...
- [generate_source/generate_yaml/generate_model/generate_snapshots (fix ordine path)](develop/macros/generate_models/generate_source.sql) — **bug**: l'ordine soprafolder/sottofolder era invertito. Se `CFG_L1_SCHEMA.cd_modulo` (chiave grezza, join key = `cd_modulo_l1`) vale es. `ANA` o `XAN`, e la lookup mappa entrambi a `CD_MODULO = ANA`, la soprafolder vera è il valore **della lookup** (`ANA`, comune), la sottofolder è il valore **grezzo** (`ANA`/`XAN`, quello che distingue) — struttura reale `OCS/ANA/{ANA,XAN}`. La versione già applicata upstream nel resync produceva invece `OCS/{ANA,XAN}/ANA` (soprafolder variabile, sottofolder sempre uguale) — **sbagliato e già live**, va riportato a monte con l'ordine corretto. Fix nelle 4 macro in `develop/macros/generate_models/`, proposto
