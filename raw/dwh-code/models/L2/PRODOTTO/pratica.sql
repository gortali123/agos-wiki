...
cte_cnslflog_co AS (
    ...
    FROM (
        SELECT
            CNSLLOG_PRATICA, CNSLLOG_DATA, CNSLLOG_ORA,
...
cte_plpratst_rt AS (
    ...
    FROM (
        SELECT
            PLPRA_NUM_PRATICA, PLPRA_UTENTE,
...
cte_plsecpra AS (
    SELECT DISTINCT
        SCPRA_NUM_PRATICA, 'S' AS FL_CARTOLARIZZATA
    FROM {{ ref('plsecpra') }}
),
...
TAB_FIN_CONSUMO AS (
    ...
    LEFT JOIN cte_oxprftafboxprftafb AS TAFB ON P.PLC_TAB_FINANZ   = TAFB.OXTB_CODICE
    ...
    LEFT JOIN {{ref ('ccanatfi')}}  AS F_CO  ON F_CO.ANATFI_FILIALE = N_PLC.INT_FILIALE
    ...
    LEFT JOIN {{ source('l1_e_bsn', 'lkp_ger_macro_prodotto_co') }} AS MPRO
        ...
        AND MPRO.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    ...
    LEFT JOIN {{ source('l1_e_bsn', 'lkp_mercato_pa_pf') }} AS MKT_PAPF
        ...
        AND MKT_PAPF.CD_SETTORE_MERCEOLOGICO = CASE
            ...
            END
        AND MKT_PAPF.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
...
cte_cnslflog_ca AS (
    SELECT
        CNSLLOG_PRATICA,
        MAX(TO_TIMESTAMP(
            TO_CHAR(CNSLLOG_DATA) || LPAD(LEFT(TO_CHAR(CNSLLOG_ORA), 6), 6, '0'),'YYYYMMDDHH24MISS')
        ) AS TS_COMUNICAZIONE_ESITO_DEF
    FROM {{ref("cnslflog")}}
...
TAB_FIN_CARTA AS (
    SELECT
        ...
        STO.DT_STORNATA                                     AS DT_STORNATA,
        STO.CD_BLOCCO_STORNO                                AS CD_BLOCCO_STORNO,
        EA.DT_ESTINZIONE_ANTICIPATA                         AS DT_ESTINZIONE_ANTICIPATA,
        EA.CD_BLOCCO_EA                                     AS CD_BLOCCO_EA,
        ...
        RE2.FL_RECUPERO,
        CRCAR.LASTMODIFIEDDATA
    FROM {{ ref('crcar') }} AS CRCAR
    ...
    LEFT JOIN {{ref ('ccanatfi')}}          AS F_CA ON F_CA.ANATFI_FILIALE  = N_INT.INT_FILIALE
    ...
    LEFT JOIN {{ source('l1_e_bsn', 'lkp_mercato_ca_b2b') }} AS MKT_B2B
        ...
        AND MKT_B2B.CD_SETTORE_MERCEOLOGICO = CASE
            ...
            END
        AND MKT_B2B.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
...
FROM {{ ref('qspra') }} A
    ...
    AND STA.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    AND STA.FL_DELETED = 'N'
    LEFT JOIN {{ ref('mfftbcaq') }} CAQ ON A.QPR_CANALE_ACQ = CAQ.MFTBCAQ_CANALE_ACQUISIZIONE
    AND CAQ.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    AND CAQ.FL_DELETED = 'N'
    LEFT JOIN {{ ref('oxisfttp') }} TTP ON A.QPR_TIPO_PROCESSO = TTP.OXISTTP_TIPO_PROCESSO
    AND TTP.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    AND TTP.FL_DELETED = 'N'
    ...
    AND TAF.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    ...
    LEFT JOIN {{ ref('oxscfpra') }} FIR ON A.QPR_NUM_PRATICA = FIR.OXSCPRA_PRATICA 
        ...
        AND FIR.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    ...
    ) ABB ON A.QPR_NUM_PRATICA = ABB.PSVT_PRATICA
    LEFT JOIN (SELECT DISTINCT CQCTASS_NUM_PRATICA, 'S' AS FL_PRODOTTO_ASSICURATIVO FROM {{ ref('cqctfass') }}
        WHERE FL_DELETED = 'N'
    ) ASS 
    ON A.QPR_NUM_PRATICA = ASS.CQCTASS_NUM_PRATICA
    LEFT JOIN (SELECT DISTINCT DPRBA_PRATICA, 'S' AS FL_RECUPERO FROM {{ ref('re2praba') }} WHERE DPRBA_PROVENIENZA = 'CQ'
    ) REC 
    ...
    ON N_INT_CQ.INT_CODICE = COALESCE(
        NULLIF(A.QPR_SUB_AGENTE, 0),
        NULLIF(A.QPR_AGENTE,     0)
    )
    LEFT JOIN {{ref ('ccanatfi')}} AS F_CQ ON F_CQ.ANATFI_FILIALE = N_INT_CQ.INT_FILIALE
    ...
    LEFT JOIN {{ source('l1_e_bsn', 'lkp_mercato_cq') }} AS MKT_AREA
        ...
        AND MKT_AREA.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
    LEFT JOIN {{ source('l1_e_bsn', 'lkp_mercato_cq') }} AS MKT_FIL
        ON N_INT_CQ.INT_FILIALE = MKT_FIL.CD_FILIALE
        AND MKT_FIL.TS_FINE_VALIDITA = TO_TIMESTAMP_NTZ('9999-12-31 00:00:00.000')
)
