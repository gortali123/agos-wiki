...
OXCTFBT_ATTIVI AS (
    ...
    FROM (
        ...
        WHERE T2.OXCTFBT_DATA_ANNULLO <> 0 AND T2.OXCTFBT_DATA_ANNULLO IS NOT NULL
    ) T
...
OXCTFBL_FB AS (
    ...
    FROM (
        ...
        WHERE T2.OXCTFBL_DATA_ANNULLO <> 0 AND T2.OXCTFBL_DATA_ANNULLO IS NOT NULL
...
OXCTFBP_ATTIVI AS (
    ...
    FROM (
        ...
        WHERE T2.OXCTFBP_DATA_ANNULLO <> 0 AND T2.OXCTFBP_DATA_ANNULLO IS NOT NULL
    ) T
    WHERE T.RN = 1
),
