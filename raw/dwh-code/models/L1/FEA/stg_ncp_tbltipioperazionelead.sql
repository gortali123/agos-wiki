select
  ...
  TRY_CAST(ds_tipo_operazione AS VARCHAR(45)) AS ds_tipo_operazione,
  TRY_CAST(ordine_tipo_operazione AS NUMBER(6,0)) AS ordine_tipo_operazione,
  TRY_CAST(eliminato AS NUMBER(1,0)) AS eliminato
from {{ source('source_l0','ncp_tbltipioperazionelead') }}
