select
  ...
  TRY_CAST(codice_domanda AS NUMBER(6,0)) AS codice_domanda,
  TRY_CAST(tipo AS NUMBER(3,0)) AS tipo,
  TRY_CAST(ds_domanda AS VARCHAR) AS ds_domanda,
  TRY_CAST(ds_domanda_dettaglio AS VARCHAR) AS ds_domanda_dettaglio,
  TRY_CAST(ordine_domanda AS NUMBER(6,0)) AS ordine_domanda,
...
from {{ source('source_l0','tblquestionaridomande') }}
