select
  ...
  TRY_CAST(NULLIF(RTRIM(AICTPDP_PROCEDURA_RIF), '') AS VARCHAR(2)) AS aictpdp_procedura_rif,
  TRY_CAST(NULLIF(RTRIM(AICTPDP_EMAIL), '') AS VARCHAR(50)) AS aictpdp_email,
  TRY_CAST(NULLIF(RTRIM(AICTPDP_IBAN), '') AS VARCHAR(34)) AS aictpdp_iban,
  TRY_CAST(NULLIF(RTRIM(AICTPDP_TIPO_INVIO), '') AS VARCHAR(1)) AS aictpdp_tipo_invio,
  TRY_CAST(NULLIF(RTRIM(AICTPDP_UTENTE), '') AS VARCHAR(10)) AS aictpdp_utente,
...
from {{ source('source_l0','aictfpdpl') }}
