select
  ...
  TRY_CAST(IFF(RTRIM(AIECPARC_TIPO_FATT) = '', ' ', RTRIM(AIECPARC_TIPO_FATT)) AS VARCHAR(2)) AS aiecparc_tipo_fatt,
  TRY_CAST(IFF(RTRIM(AIECPARC_TIPO_GENERAZ) = '', ' ', RTRIM(AIECPARC_TIPO_GENERAZ)) AS VARCHAR(1)) AS aiecparc_tipo_generaz,
  TRY_CAST(IFF(RTRIM(AIECPARC_PROC_RIF) = '', ' ', RTRIM(AIECPARC_PROC_RIF)) AS VARCHAR(2)) AS aiecparc_proc_rif,
