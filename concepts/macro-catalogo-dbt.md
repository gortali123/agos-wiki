...
- `generate_model` — genera il corpo SQL del modello staging L1 (campi tecnici condizionati dal cluster, `TRY_CAST` per colonna, gestione valori vuoti OCS→spazio invece di NULL). **Fix proposto** (2026-07-14, non ancora applicato upstream): l'espressione per colonne VARCHAR OCS valutava `RTRIM(col)` due volte (`TRY_CAST(IFF(RTRIM(col)='', ' ', RTRIM(col)) AS T)`); in `develop/macros/generate_models/generate_model.sql` è stata riscritta come `COALESCE(NULLIF(TRY_CAST(RTRIM(col) AS T), ''), ' ')` (RTRIM valutato una sola volta). Discusso con l'utente il trade-off: questa forma fa sì che un overflow di lunghezza sul valore *già trimmato* produca uno spazio invece di NULL — accettato perché il vero controllo di business ("la stringa con gli spazi di padding non deve superare la lunghezza dichiarata") è demandato al test generico `try_cast` (validato sul valore grezzo di L0, non trimmato) e non al modello — vedi nota sotto su `try_cast`.
