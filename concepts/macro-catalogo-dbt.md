...
Catalogo delle macro DBT presenti in `raw/dwh-code/macros/`, riverificato per intero il 2026-08-19 (spot-check aggiornato: `custom_to_date`, `get_dt_osservazione`/`compute_dt_osservazione`/`delete_dt_osservazione` — **`delete_month` e `last_day_past_month` non esistono più/mai esistiti nel codice, sostituiti da questa terna**, vedi sotto —, `is_incremental_S1`, `ts_fine_validita`, `scd2_foto_mensile`, `add_datamask`/`remove_datamask` assente, `log_run_results`, `call_proc_report_fondi_masterscale`). Le macro documentate nei tre file `raw/` sono confermate quasi tutte presenti, con alcune differenze di naming residue — vedi [[inconsistenze]].
...
- **Cluster D (`get_dt_osservazione`/`compute_dt_osservazione`/`delete_dt_osservazione`)** — verificate riga per riga 2026-08-19, ora anche correttamente documentate in [[caricamento-layer-l0-l1]] (nessuna incongruenza residua, non più `delete_month` come nome):
  - `get_dt_osservazione(schedule='monthly')` — calcola il periodo "corrente" rispetto a `CURRENT_DATE`: per `'monthly'` l'ultimo giorno del mese precedente (`LAST_DAY(DATEADD(MONTH,-1,CURRENT_DATE))`), per `'weekly'` il venerdì precedente. Nessun parametro di timestamp/colonna in input — solo override manuale via `var('dt_osservazione')`.
  - `compute_dt_osservazione(column, schedule='monthly')` — usata in SELECT per derivare `DT_OSSERVAZIONE` da una colonna data/timestamp sorgente: per `'monthly'` la riporta a fine mese se non lo è già, per `'weekly'` al venerdì della settimana ISO.
  - `delete_dt_osservazione(column='DT_OSSERVAZIONE', schedule='monthly')` — pre-hook, `DELETE FROM {{ this }} WHERE {{ column }} = {{ get_dt_osservazione(schedule) }}` (solo se la relation esiste già). Il parametro `column` è usato correttamente nel WHERE (non hardcoded).
