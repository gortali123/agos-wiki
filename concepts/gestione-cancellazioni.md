...
**Bug noto**: `delete_l2` (`raw/dwh-code/macros/logic_delete/delete_l2.sql`) non ha un guard `{% if is_incremental() %}` — al primo run di un'entità (o dopo `--full-refresh`), `{{ this }}` non esiste ancora e il `DELETE FROM {{ this }}` del pre-hook fallisce. Verificato su tutti i ~15 modelli che lo usano come pre-hook: nessuno lo condiziona lato chiamante. Ancora aperto lato codice; la nota di diagnosi/fix proposto in `queries/` è stata rimossa dopo la chiusura del punto.
