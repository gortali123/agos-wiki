{#
  ...
  Args:
    date_expr (opzionale): settimana da eliminare. Default: get_dt_settimana().

  - modelli L1: passare get_dt_settimana('ts_riferimento');
  - modelli L2/L3: non passare nulla.
  Gira solo se la tabella esiste gia' (skip al primo run / full-refresh).
#}
{% macro delete_week(column='DT_ACCETTAZIONE', date_expr=get_dt_settimana()) %}
{% if execute %}
