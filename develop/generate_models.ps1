...
function Invoke-Dbt {
    ...
    # dbt run-operation puo' terminare con exit code 0 anche quando l'output contiene
    # un errore di compilazione/database (eccezione catturata internamente da dbt senza
    # propagare l'exit code) - senza questo controllo lo script prosegue silenziosamente.
    $errorLine = $output | Where-Object { $_ -match '(Compilation Error|Database Error|Runtime Error|Encountered an error)' } | Select-Object -First 1

    if ($exitCode -ne 0 -or $errorLine) {
